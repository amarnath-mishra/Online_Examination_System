...
public class HandleExam1 extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        ArrayList<QuesList> questn;
        ArrayList<QuesList> questn1 = new ArrayList<QuesList>();
        questn = (ArrayList<QuesList>) req.getSession().getAttribute("questions");

        Iterator it = questn.iterator();
        String temp;
        QuesList qlist = null;
        int counter = 0;
        ...
        while (it.hasNext()) {
            counter = counter + 1;
            qlist = (QuesList) it.next();
            int q = qlist.getquesid();
            out.println("yo " + q + "\n and counter =" + counter);
            if (id2 == q) {
                Status = qlist.getAnswered();
                if (Status.equals("not answered")) {
                    qlist.setAnswered("answered "+ choice);
                } else if (Status.equals("answered")) {
                    qlist.setAnswered("changedAnswer "+choice);
                } else {
                    qlist.setAnswered("Changed_answer "+choice);
                }
            }
            questn1.add(qlist);
        }
        ...
        // out.println("ForWAEDD TO vIEWExamqn first");

        // out.println("\n  bbb End counter ="+counter);
        req.getSession().setAttribute("questions", questn1);
        out.println("ForWAEDD TO vIEWExamqn");
